...
class FilterView(ConfigurableMixin, QWidget):
    class RateView(TableView):
        ...
        def update_prices(self, prices: List[dict] = ()):
            ...
            if self.rowCount() != row_n:
                if self.rowCount() > row_n:
                    self.row_unmatch += 1
                    print('#', list(sorted(map(lambda x: x['service'], bids))))
    ...
    def select_instrument(self, instrument: str):
        ...
        self.open_spread_spin.setValue(fltr.get('open_spread', 3.0))
        self.close_spread_spin.setValue(fltr.get('close_spread', 3.0))
        self.open_duration_spin.setValue(fltr.get('open_duration', 9.9))
        self.close_duration_spin.setValue(fltr.get('close_duration', 9.9))
    ...
    def __init__(self, *, name: str):
        ...
        open_spread_spin = DoubleSpinBox(value=3.0, min=0.0, max=9.99, decimals=2, single_step=0.01, suffix=' pips',
                                         name=name + '.open_spread')
        close_spread_spin = DoubleSpinBox(value=3.0, min=0.0, max=9.99, decimals=2, single_step=0.01, suffix=' pips',
                                          name=name + '.close_spread')
        open_duration_spin = DoubleSpinBox(value=5.0, min=0.0, max=9.99, decimals=1, single_step=0.1, suffix=' s',
                                           name=name + '.open_duration')
        close_duration_spin = DoubleSpinBox(value=5.0, min=0.0, max=9.99, decimals=1, single_step=0.1, suffix=' s',
...
class AppMaster(Master):
    ...
    def get_prices(self):
        ...
        if is_timeout:
            ...
            gevent.sleep(max((timedelta(seconds=Config()['interval']) - elapsed).total_seconds(), 0.05))
